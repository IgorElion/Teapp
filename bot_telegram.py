...
ATIVOS_CATEGORIAS = {
    ...
    "ETH(OTC)": "Blitz", "BNB(OTC)": "Blitz", "XRP(OTC)": "Blitz",
    "ADA(OTC)": "Blitz", "DOGE(OTC)": "Blitz", "SOL(OTC)": "Blitz",
    "MATIC(OTC)": "Blitz", "DOT(OTC)": "Blitz", "AVAX(OTC)": "Blitz",
    "TRX(OTC)": "Blitz", "LINK(OTC)": "Blitz", "ATOM(OTC)": "Blitz",
    "LTC(OTC)": "Blitz", "NEAR(OTC)": "Blitz", "UNI(OTC)": "Blitz",
    "ETC(OTC)": "Blitz", "BCH(OTC)": "Blitz", "ALGO(OTC)": "Blitz",
    "XLM(OTC)": "Blitz",
...
def send_message():
    ...
    try:
        ...
        gale1_time = entry_time + timedelta(minutes=3)
        gale2_time = gale1_time + timedelta(minutes=3)
        ...
        # Define a expiração com base na categoria (lógica simplificada)
        if categoria == "Blitz":
            expiracao_segundos = random.choice([5, 10, 15, 30])
            expiracao_texto = f"⏳ Expiração: {expiracao_segundos} segundos"
        elif categoria == "Digital":
            minutos_expiracao = random.choice([1, 3, 5])  # Alterado: limitado a 5 minutos máximo
            expiracao_time = entry_time + timedelta(minutes=minutos_expiracao)
            expiracao_texto = f"⏳ Expiração: {minutos_expiracao} minutos ({expiracao_time.strftime('%H:%M')})"
        elif categoria == "Binary":
            expiracao_time = entry_time + timedelta(minutes=1)
        ...
        message = (
            ...
            f"{expiracao_texto}\n"
            f"Gale 1 - {gale1_time.strftime('%H:%M')}\n"
            f"Gale 2 - {gale2_time.strftime('%H:%M')}\n"
            "\n👉🏻 Clique aqui para abrir a corretora: https://trade.xxbroker.com/register?aff=436564&aff_model=revenue&afftrack="
        )
        ...
        if response.status_code == 200:
            ...
            logging.error(f"Falha ao enviar mensagem. Erro: {response.status_code} - {response.text}")
...
def schedule_messages():
    ...
    # Agenda para horários exatos com 1 segundo de atraso a cada 6 minutos durante todo o dia
    for hora in range(24):  # 0 a 23 horas
        for minuto in [0, 6, 12, 18, 24, 30, 36, 42, 48, 54]:  # A cada 6 minutos
            # Adiciona 1 segundo para evitar problemas de arredondamento no Telegram
            horario_formatado = f"{hora:02d}:{minuto:02d}:01"  # Adicionado :01 (1 segundo)
            schedule.every().day.at(horario_formatado).do(send_message)
            logging.info(f"Sinal agendado para {horario_formatado}")
    
    logging.info("Bot agendado para enviar sinais a cada 6 minutos em horários exatos (com 1 segundo de atraso).")
...
def keep_bot_running():
    ...
    # Adiciona 1 segundo ao horário exato para evitar arredondamento no Telegram
    proximo_horario = agora.replace(minute=proximo_minuto, second=1, microsecond=0)
